...
# I can almost Guarantee that your instructor drilled into you the
# belief that 'Goto' is the root of all evil. Most modern languages hide
# the fact if they support 'Goto' at all.
# This a problem, because in Assembly, nearly all logic is done with
...
#
# I'm not claiming that 'Goto's Arn't Evil....it just a type of evil
# you are just going to have to learn to deal with.
#
# Now looping 'forever' is not a good idea, (in most cases) so I have
# to introduce looping with at least some way to 'exit' the loop, so we
...
@POPI LoopCount
#
# Now we set a label to where our loop will return each time.
...
:LoopStart
#
# do our loop body...in this case print the counter, followed by a space.
@PRTI LoopCount  @PRT " "
#
# Now add one to LoopCount, we could push, add, and pop it but we have
...
# '20'
#
# So use a CMP (compare) function to compare LoopStart to 20
@PUSH 20
@CMPI LoopCount
...
# Our Print out actual went from 0 to 19 rather than 1 to 20.
