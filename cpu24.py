...
class AssemblerContext:
    def __init__(self):
        ...
        self.GlobalDeclarations={"END__", "__CEND", "__DEND"}
        self.GlobalDeclInfo={}
        self.StructStack = []
...
# Experimental 24-bit/Ring 1 instructions. Keep the legacy CPU.json unchanged.
for _op in (
...
class microcpu:
    ...
    def insertbyte(self, location, value, context=None):
        ...
        if location < 0 or location >= len(self.memspace):
            self.raiseerror("190 Physical Address OverFlow %06x" % location)
    ...
    def physical_address(self, segment, offset):
        return ((int(segment) & 0xff) << 16) | (int(offset) & 0xffff)

    def code_address(self, offset):
        segment = self.CS if self.SegmentMode else 0
        return self.physical_address(segment, offset)

    def active_data_segment(self):
    ...
    def data_address(self, offset):
        return self.physical_address(self.active_data_segment(), offset)
    ...
    def getwordat(self, address):
        ...
        if address <= MAXMEMSP:
            if address == MAXMEMSP:
                self.raiseerror("220 Word access crosses segment boundary")
            address = self.data_address(address)
        if address < 0 or address + 1 >= len(self.memspace):
            self.raiseerror("220 Invalid Physical Address: %d, getwordat" % address)
        return self.getwordmem(address)

    def putwordat(self, address, value):
        address = int(address)
        if address <= MAXMEMSP:
            if address == MAXMEMSP:
                self.raiseerror("230 Word access crosses segment boundary")
            address = self.data_address(address)
        if address < 0 or address + 1 >= len(self.memspace):
            self.raiseerror("230 Invalid Physical Address: %d, putwordat" % address)
    ...
    def optPUSHI(self, address):
        ...
        self.hwstack[sp] = self.getwordat(address)
        self.hwstacksp += 1
    ...
    def optPOPI(self, address):
        ...
        self.putwordat(address, self.hwstack[sp])
        self.hwstacksp -= 1
    ...
    def optCAST(self, address):
        global Debug,  PrevPC
        address = self.data_address(address)
        bank_end = (address & 0xff0000) | 0xffff
        ...
        if cmd == CastPrintStr:
            ...
            i = address
            while self.memspace[i] != 0 and i < bank_end:
                c = self.memspace[i]
                if c == 0:
        ...
        if cmd == CastPrintStrI:
            ...
            # active data segment, not a physical bank-0 address.
            i = self.data_address(self.getwordmem(address))
            while i < bank_end and self.memspace[i] != 0:
                c = self.memspace[i]
                if (c < 32 ) and c not in (7, 8, 9, 10, 13, 27, 30):
                    sys.stdout.write("%02x" % c)
                else:
        ...
        if cmd == PollReadSector:
            ...
            if current_context.DeviceHandle != None:
                ...
                if v <= bank_end-0x1ff:
                    ...
                    self.raiseerror(
                        "042 Attempted to read block with insuffient memory %04x < 0x4x" %(v,bank_end-0xff))
            else:
        ...
        if cmd == PollReadSectorI:
            ...
            if current_context.DeviceHandle is not None:
                ...
                if v <= bank_end-0x1ff:
                    ...
                    self.raiseerror(f"550 Attempted to read block with insuffient memory {v:04x} < {bank_end-0xff}")
        if cmd == PollReadTape:
            self.optPOPNULL(address)            
            if current_context.DeviceHandle is not None:
                v=address
...
# PollReadTime should be reworked to write time to 32 bit block at address rather than stack.
    ...
    def optADM(self,address):
        # Ring 1 prototype: consume the requested mode from TOS.
        if self.hwstacksp < 1:
            self.raiseerror("580 ADM requires mode on stack")
        mode = self.fetchStack(0)
        self.hwstacksp -= 1
        if mode == 0:
            self.SegmentMode = 0
            self.CS = self.DS = self.ES = 0
            self.segment_override = None
        elif mode == 1:
            self.SegmentMode = 1
            self.segment_override = None
        else:
            self.raiseerror("581 ADM mode must be 0 or 1")

    def optCSO(self, address):
        self.segment_override = self.CS if self.SegmentMode else 0
        self.prefix_just_set = True

    def optDSO(self, address):
        self.segment_override = self.DS if self.SegmentMode else 0
        self.prefix_just_set = True

    def optESO(self, address):
        self.segment_override = self.ES if self.SegmentMode else 0
        self.prefix_just_set = True

    def optSSO(self, address):
        self.raiseerror("582 SSO requires Ring 2")

    def optSGET(self, selector):
        registers = (self.CS, self.DS, self.ES)
        if selector < 0 or selector >= len(registers):
            self.raiseerror("583 Invalid segment selector")
        self.optPUSH(registers[selector])

    def optSSET(self, selector):
        if selector < 0 or selector > 2:
            self.raiseerror("584 Invalid segment selector")
        if self.hwstacksp < 1:
            self.raiseerror("585 SSET requires value on stack")
        value = self.fetchStack(0)
        self.hwstacksp -= 1
        if value > 0xff:
            self.raiseerror("586 Segment value exceeds 8 bits")
        if selector == 0:
            self.raiseerror("587 CS cannot be changed with SSET")
        if selector == 1:
            self.DS = value
        else:
            self.ES = value
    ...
    def _evalpc_py(self, context, dosteps):
        ...
        if dosteps == -1:
            while True:
                pc = self.pc & 0xffff
                PrevPC = pc                
                physical_pc = self.code_address(pc)
                optcode = mem[physical_pc]
                ...
                if size == 3:
                    if pc > 0xfffc:
                        raiseerror("588 Instruction operand crosses CS boundary")
                    argument = self.getwordmem(physical_pc + 1)
                else:
                    argument = 0
...
def execute_assembler_command(cmd, CPU, context):
    ...
    if key[0] == ":":
        ...
        if value > 0xff:
            CPU.raiseerror(
                f".DATA bank must be 0..255, got {value} "
                f"{filename}:{context.FileLineNum}"
            )
        context.DataSegment = value
        context.InitialDS = value
        context.dataaddress = value << 16
        return ""
    elif key.upper() == ".DATACOPY":
        if not rest:
            CPU.raiseerror(f"Missing bank for .DATACOPY {filename}:{context.FileLineNum}")
        value, used = FirstPassVal(
            rest,
            context,
            filename=filename,
        ...
        if trailing:
            CPU.raiseerror(
                f"Unexpected trailing text after .DATACOPY: {trailing!r} "
                f"{filename}:{context.FileLineNum}"
            )
        value = Str2Word(value)
        if value == 0 or value > 0xff:
            CPU.raiseerror(
                f".DATACOPY bank must be 1..255, got {value} "
                f"{filename}:{context.FileLineNum}"
            )
        context.DataCopyBank = value
        context.InitialDS = value
        return ""
    elif key == "D":
...
def resolve_all_forward_references(context, CPU):
    ...
    # Publish independent logical high-water marks for code and fixed data.
    # END__ remains the legacy alias for the code end.
    code_end = max(context.codehighaddress, context.address) & 0xffff
    data_end = context.dataaddress & 0xffff if context.DataSegment != -1 else code_end
    for symbol, value in (("END__", code_end), ("__CEND", code_end),
                          ("__DEND", data_end)):
        context.GlobeLabels[symbol] = value
        context.FileLabels[symbol] = value
        context.DefinedSymbols.add(symbol)
        LocVarHist[symbol] = [{"value": value, "start": 0, "end": None}]
...
#    Entry = 0
    ...
    i = 0
    SP = -1
    if context.DataCopyBank is not None:
